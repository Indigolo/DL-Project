...
if torch.cuda.is_available():
    try:
        try:
            ...
            include_paths = torch.utils.cpp_extension.include_paths(device_type="cuda")
        os.environ["CUDA_LIB"] = os.path.join(os.path.split(include_paths[-1])[0], "lib")
    except OSError:
        # No CUDA toolkit (nvcc/CUDA_HOME) installed. Harmless unless the
        # "cuda" sLSTM backend is actually JIT-compiled; the "vanilla" backend
        # never calls load() and doesn't need CUDA_LIB.
        pass
