...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.patch_num = (
            configs.seq_len - self.patch_size
        ) // self.stride + 1  # the number of patches
        ...
        cfg = xLSTMBlockStackConfig(
            ...
            slstm_block=sLSTMBlockConfig(
                ...
                feedforward=FeedForwardConfig(proj_factor=1.3, act_fn="gelu", dropout=configs.dropout),
            ),
        ...
        # channel mixing (xLSTM-Mixer, arXiv:2410.16928): each channel's encoded
        # patches are collapsed to one token, and a second sLSTM stack strides
        # over the channel axis instead of the time axis, so channels exchange
        # information through the same recurrent scalar-memory mechanism that
        # normally only mixes information across time. Off by default so every
        # existing checkpoint/run is unaffected.
        self.channel_mixing = getattr(configs, "channel_mixing", False)
        if self.channel_mixing:
            self.channel_proj = nn.Linear(self.embedding_dim * self.patch_num, self.embedding_dim)
            mix_cfg = xLSTMBlockStackConfig(
                slstm_block=sLSTMBlockConfig(
                    slstm=sLSTMLayerConfig(
                        num_heads=configs.num_heads,
                        backend="vanilla",
                ...
                context_length=self.channel,
                num_blocks=1,
                embedding_dim=self.embedding_dim,
                dropout=configs.dropout,
                slstm_at="all",
            )
            self.channel_mixer = xLSTMBlockStack(mix_cfg)
            self.projection = nn.Linear(self.embedding_dim, self.pred_len)
        else:
    ...
    def forward(self, x):
        ...
        x = self.xlstm_stack(x)  # [(B * C), Num_patches, Embedding_dim]  -- per-channel, unchanged

        x = x.flatten(1)  # [(B * C), Num_patches * Embedding_dim]

        if self.channel_mixing:
            x = self.channel_proj(x)  # [(B * C), Embedding_dim] -- one token per channel
            x = rearrange(x, "(b c) d -> b c d", b=B, c=C)  # channels become the sequence axis
            x = self.channel_mixer(x)  # sLSTM recurrence strides across channels -- the mixing step
